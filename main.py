...
def escanear_pasta():
    estado_atual = {}
    for raiz, _,  arquivos in os.walk(PASTA_COMPARTILHADA):#tem o _ pq nao vai precisar usar as subpastas
...
def iniciar_servidor_tcp():
    ...
    while True:
        ...
        threading.Thread(target=lidar_com_cliente, args=(cliente_socket,), daemon=True).start()
...
def monitor_eventos():
    ...
    while True:
        ...
        for antigo, novo in arquivos_renomeados:
            eventos_tracker.append({"acao": "RENAME", "arquivo": antigo, "novo_nome": novo})
        for arq in arquivos_deletados: 
            eventos_tracker.append({"acao": "DELETE", "arquivo": arq})
        for arq in arquivos_novos: 
            eventos_tracker.append({"acao": "CREATE", "arquivo": arq, "tamanho": estado_atual[arq]['tamanho']})
        for arq in arquivos_modificados: 
            eventos_tracker.append({"acao": "MODIFY", "arquivo": arq, "tamanho": estado_atual[arq]['tamanho']})
